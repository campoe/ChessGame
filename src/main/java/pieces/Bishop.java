...
public class Bishop extends Piece {
    ...
    private static final int[] DELTAS = new int[]{-9, -7, 7, 9};

    public Bishop(int id, int row, int column, Alliance alliance) {
    ...
    private boolean isFirstColumn(int position, int delta) {
        return (position % 8 == 0) && (delta == -9 || delta == 7);
    }

    private boolean isEightColumn(int position, int delta) {
        return (position % 8 == 7) && (delta == -7 || delta == 9);
    }

    public String toString() {
        return alliance.toString() + "BI" + id;
    }
